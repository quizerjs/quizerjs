...
describe('TextInputTool', () => {
  ...
  describe('构造函数', () => {
    ...
    it('应该使用提供的数据创建实例', () => {
      ...
      expect(tool['data'].question.text).toBe('测试文本输入题');
      expect(tool['data'].question.correctAnswer).toBe('正确答案');
    });
  });
  ...
  describe('validate', () => {
    ...
    it('应该拒绝空问题文本', () => {
      const data = createTextInputData({ question: { text: '' } });
      const tool = new TextInputTool({ data });
      const isValid = tool.validate(data);
    ...
    it('应该拒绝空正确答案', () => {
      const data = createTextInputData({ question: { correctAnswer: '' } });
      const tool = new TextInputTool({ data });
      const isValid = tool.validate(data);
      expect(isValid).toBe(false);
    });
    ...
    it('应该接受有效的文本输入题', () => {
      const data = createTextInputData({
        question: { text: '问题', correctAnswer: '答案' },
      });
      const tool = new TextInputTool({ data });
      const isValid = tool.validate(data);
      expect(isValid).toBe(true);
